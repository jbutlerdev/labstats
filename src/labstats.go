...
func (s *StatusServer) Start() {
	...
	pb.RegisterLabStatsServer(s.grpcServer, &server{StatusServer: s})
	s.ConnectRedis()
...
func (s *server) Stats(ctx context.Context, in *pb.MachineState) (*pb.Empty, error) {
	...
	err := s.StatusServer.redisClient.Set(s.StatusServer.redisClient.Context(), "machineName", in.GetName(), 0).Err()
	return &pb.Empty{}, err
}
